...
for subf_in = 1:numel(subfolders)
    ...
    if subfolders(subf_in).isdir && ~startsWith(subfolders(subf_in).name, '.') && startsWith(subfolders(subf_in).name, 'Conf')
        ...
        for file_id= 1:length(qpos_sel)
            ...
            % K_x = polyfit(dxy_vec(1, :), F_vec(1, :), 1);
            % K_y = polyfit(dxy_vec(2, :),F_vec(2, :), 1);
            % K_endps = [K_x,mdl = fitlm(d.', -F.');K_y];

             % K_endps = -F_vec(:,:)*(dxy_vec(:,:))';  % Obs, it's not square

            % K_endps_mdl = zeros(size(F_vec, 1), size(dxy_vec, 1));
            ...
            for i = 1:size(F_vec, 1)
                ...
                % I.W.: the following can be used to visualize the fit
                % figure()
                % plot(mdl_nointercept)
                % K_endps_mdl(i, :) = mdl.Coefficients.Estimate(2:end).'; 
                K_endps_mdl_nointercept(i, :) = mdl_nointercept.Coefficients.Estimate(1:end).';
                % disp(['Root mean square error in it = ',num2str(i), ' is: ',num2str(mdl_nointercept.RMSE)])
            ...
            if file_id==7
                   ...
                   % Plot specifically X vs Y for file_id=forcce_level (e.g. 0.5N=1, 0.75=2N,... etc)
                    
                    XY2N  = figure('Name', 'X vs Y at 2'  ,'Position', [0 0 1500 1500],'NumberTitle', 'off','color','w');
                    ...
                    % max_axis = max([disx_f,disy_f],[],'all');
                    % axis([-max_axis max_axis -max_axis max_axis])

                    axis square
                    legend('02 N','Endpoint Position')
                    saveas(XY2N,strcat('2N',' X vs Y_',subfolders(subf_in).name(6:8),'.fig') )
                    saveas(XY2N,strcat('2N',' X vs Y_',subfolders(subf_in).name(6:8),'.png') )


                    % Plot specifically Force vs Displacment for file_id=forcce_level (e.g. 0.5N=1, 0.75=2N,... etc)
                    SE2N  = figure('Name', 'Stiffness Ellipsoid at 2N'  ,'Position', [0 0 9000 9000],'NumberTitle', 'off','color','w');
                    ...
                    legend('02 N','Endpoint Position')
                    saveas(SE2N,strcat('2N',' SE_',subfolders(subf_in).name(6:8),'.fig') )
                    saveas(SE2N,strcat('2N',' SE_',subfolders(subf_in).name(6:8),'.png') )
