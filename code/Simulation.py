...
def controller_pert(model, data, initial_activations, final_activations):
    # put the controller here. This function is controlled inside the simulation.
    global runtime, start_time, k, n, theta_degrees, p, flag, force_x, force_y, flag1, elv_angle, shoulder_elv, shoulder_rot, elbow_flex, pro_sup
    if runtime <= k*hold_duration-5.9:
        ...
        # Detect whether the limits of one of the selected joints are exceeded
        ...
        if runtime >= k*hold_duration-6:
            ...
            shoulder_elv = int(shoulder_elv.data)
            shoulder_rot = data.sensor('shoulder_rot_sensor')
            shoulder_rot = int(shoulder_rot.data)     d
            elbow_flex = data.sensor('elbow_flexion_sensor')
            elbow_flex = int(elbow_flex.data)
            pro_sup = data.sensor('pro_sup_sensor')
            pro_sup = int(pro_sup.data)
    ...
    mj.mj_step(model, data)
    return p, elv_angle, shoulder_elv, shoulder_rot, elbow_flex, pro_sup
    ...
    while not glfw.window_should_close(window):
        ...
        while (data.time - time_prev < 1.0/120):
            ...
            if counter<250:
                ...
                p, elv_angle, shoulder_elv, shoulder_rot, elbow_flex, pro_sup = controller_pert(model, data, initial_activations, final_activations)
